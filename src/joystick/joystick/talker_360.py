...
"""
@file
This file contains the node that convert a generic XBOX 360 controller input to specific Float32MultiArray message.
...
## @brief Node that reads the XBOX 360 joystick status and publishes it to joystatus topic
#
...
class Talker360(Node) :
    ## Constructor
    def __init__(self):
        super().__init__("talker_360")
    ...
    #
    # Publish a message to /joystatus topic only if the controller is connected, the sticks have been moved in all the direction and then released.
    ...
    def joyread_callback(self, data):
        ...
        # Forward/Backward
        status.data[movements.get("x_movement")] = 0# To be Completed
        # Turn Left/Right
        status.data[movements.get("z_rotation")] = 0# To be Completed
...
## @brief Main function
#
# Initialize ROS2 and start the Talker360 node. Shuts down with CTRL-C.
def main(args=None):
    # Initialize ROS2
    rclpy.init(args=args)
    # Create the node
    node = Talker360()
    node.get_logger().info('Starting X360 joystick node, shut down with CTRL-C')
